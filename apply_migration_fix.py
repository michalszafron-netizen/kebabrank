...
def apply_migration():
    ...
    # SQL to drop and recreate the function
    sql = """
    DROP FUNCTION IF EXISTS get_city_rankings_with_latest_ratings;
    
    CREATE OR REPLACE FUNCTION get_city_rankings_with_latest_ratings(
        p_city_id integer,
        p_limit integer,
        p_offset integer
    )
    ...
    try:
        # Execute the SQL
        result = db.client.rpc('execute_sql', {'sql': sql}).execute()
