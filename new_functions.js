...
function displayRankings(rankings, container, cityName, isGlobal = false) {
    ...
    if (isGlobal) {
        // html += `<h2 class="text-xl font-bold mb-4 px-2">Top 10 in Poland</h2>`;
    }
    ...
    sortedRankings.forEach((kebab, index) => {
        ...
        html += `
            <div class="glass-card rounded-xl p-4 flex gap-4 relative overflow-hidden group cursor-pointer hover:border-primary/30 mb-4 transition-all"
                        ...
                        <div class="flex justify-between items-start">
                             ...
                             <div class="flex items-center gap-2">
                                <div class="px-2 py-1 rounded bg-[#1e293b] border border-white/10 flex flex-col items-end">
                                    ...
                                    <span class="text-sm font-black text-primary">${kebab.rank_score ? kebab.rank_score.toFixed(1) : 'N/A'}</span>
                                </div>
                             </div>
                        </div>
                        <p class="text-[10px] text-gray-400 mt-1 flex items-center gap-1 truncate mb-2">
                            <span class="material-icons text-[10px]">place</span> ${kebab.address}, ${kebab.city || window.currentCity || ''}
                        </p>
                    </div>

                    <!-- Stats Row -->
                    <div class="flex items-center gap-4 text-[10px] text-gray-300 mb-3 border-b border-white/5 pb-2">
                        <div class="flex items-center gap-1">
                            <span class="material-icons text-yellow-500 text-sm">star</span>
                            <span class="font-bold text-white text-xs">${kebab.rating.toFixed(1)}</span>
                            <span class="text-gray-500">Google Rating</span>
                        </div>
                         <div class="flex items-center gap-1">
                            <span class="material-icons text-blue-400 text-sm">reviews</span>
                            <span class="font-bold text-white text-xs">${kebab.total_reviews.toLocaleString()}</span>
