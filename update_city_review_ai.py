...
def update_city_review_ai(city_name: str, limit: int = 10, skip_existing: bool = False):
    load_dotenv()
    ...
    try:
        ai_updater.update_ai_analysis_for_city(city_name, limit=limit, skip_existing=skip_existing)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--skip-existing', action='store_true', 
                        help='Skip kebabs that already have AI analysis (saves API tokens)')
    args = parser.parse_args()
    
    for city in args.cities:
        update_city_review_ai(city, args.limit, skip_existing=args.skip_existing)
